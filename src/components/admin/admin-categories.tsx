...
export function AdminCategories() {
  ...
  const [form, setForm] = useState({ name: "", slug: "", description: "", image: "", brand: "BOTH", sortOrder: "0" });
  ...
  const openNew = () => { setEditing(null); setForm({ name: "", slug: "", description: "", image: "", brand: "BOTH", sortOrder: "0" }); setOpen(true); };
  const openEdit = (c: Category) => { setEditing(c); setForm({ name: c.name, slug: c.slug, description: c.description || "", image: c.image || "", brand: c.brand, sortOrder: String(c.sortOrder) }); setOpen(true); };
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <span className={cn("inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full",
                      c.brand === "SAVERS" ? "brand-savers" : c.brand === "CHOVIL" ? "brand-chovil" : "bg-primary/10 text-primary")}>
      ...
      <Dialog open={open} onOpenChange={(o) => { if (!o) { setEditing(null); setForm({ name: "", slug: "", description: "", image: "", brand: "BOTH", sortOrder: "0" }); } setOpen(o); }}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto z-[110]" overlayClassName="z-[110]">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>توضیحات</Label>
              <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} maxLength={1000} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: e.target.value })} dir="ltr" type="number" />
              </div>
            </div>
            <ImagePicker value={form.image} onChange={(v) => setForm({ ...form, image: v })} label="تصویر دسته" />
          </div>
